...
class single_LSTM(nn.Module):
    def __init__(self, n_classes=2, input_size=512 * 3 * 9 * 9, hidden_size=2, n_layers=1):
        ...
        self.lstm = nn.LSTM(input_size, hidden_size, batch_first=True, bidirectional=False) # one direction
    ...
    def forward(self, x):
        ...
        h_0 = torch.zeros(self.n_layers*1, n_batch, self.hidden_size).to(device)
        c_0 = torch.zeros(self.n_layers*1, n_batch, self.hidden_size).to(device)
