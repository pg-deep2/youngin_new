...
def P3D199(pretrained=False, modality='RGB', **kwargs):
    ...
    if pretrained == True:
        if modality == 'RGB':
            pretrained_file = '../pretrained-weight/p3d_rgb_199.checkpoint.pth.tar'
        elif modality == 'Flow':
            pretrained_file = '../pretrained-weight/p3d_flow_199.checkpoint.pth.tar'
...
if __name__ == '__main__':
    ...
    # hPath = r"C:\Users\young\Desktop\pg_young\Highlight_TEKKEN\dataset\HV"
    # rPath = r"C:\Users\young\Desktop\pg_young\Highlight_TEKKEN\dataset\RV"

    # Dataloader 가져오기
    h_loader, r_loader = get_loader()

    for idx, (frames, scores) in enumerate(r_loader):
        # VideoLoader.plotVideo(frames)
        break

    # pre-trained net 통과
    frames = frames[:, :50, :, :, :] # 시간 오래 걸리니까 조금만 잘라서
    frames = frames.permute(0, 2, 1, 3, 4)
    print("input:", frames.shape)
    out = model(frames)
    print(out.size(), type(out))
    # ==> 이 코드는 돌아가기는 하는데 depth이 줄어듦 50->48 이런 식으로
    # ==> 근데 위에 있는 torch.rand(10, 3, 16, 160, 160)은 depth이 줄어들지 않았는데 왜 이것만??